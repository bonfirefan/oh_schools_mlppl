...
CREATE MATERIALIZED VIEW sketch.ode_school AS
select * from
	ohschools.ode_type t
left join
	ohschools.ode_type_ref r
on t.typology_2013 = r.SchType
left outer join
	ohschools.ode_profile p
using(IRN)
...
from
	...
	grant select on sketch.train_data to public;
	grant delete on sketch.train_data to cchallu;
	grant delete on sketch.train_data to sduran;
	grant delete on sketch.train_data to byfan;
